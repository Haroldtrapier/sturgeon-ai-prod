  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="text-stone-500 mt-1">Configure alerts, matching, and data source preferences</p>
      </div>
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={opt.key} className="flex items-center justify-between">
                ...
                <button onClick={() => setSettings(prev => ({ ...prev, [opt.key]: !prev[opt.key as keyof typeof prev] }))} className={`w-11 h-6 rounded-full transition-colors relative ${settings[opt.key as keyof typeof settings] ? "bg-lime-700" : "bg-stone-200"}`}>
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between mb-1">
                ...
                <span className="text-sm text-lime-700">{settings.match_threshold}%</span>
              </div>
              ...
              <p className="text-xs text-stone-500 mt-1">Only show opportunities scoring above this threshold</p>
            </div>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium">Auto-Save Matches</p>
                <p className="text-xs text-stone-500">Automatically save high-scoring opportunities</p>
              </div>
              <button onClick={() => setSettings(prev => ({ ...prev, auto_save_matches: !prev.auto_save_matches }))} className={`w-11 h-6 rounded-full transition-colors relative ${settings.auto_save_matches ? "bg-lime-700" : "bg-stone-200"}`}>
            ...
            <div>
              ...
              <select value={settings.alert_before_deadline} onChange={e => setSettings(prev => ({ ...prev, alert_before_deadline: parseInt(e.target.value) }))} className="mt-1 w-full px-4 py-2 bg-stone-100 border border-stone-300 rounded-lg focus:ring-2 focus:ring-lime-500 focus:outline-none text-sm">
          ...
          <div className="space-y-3">
            {SOURCES.map(src => (
              <div key={src.id} className="flex items-center justify-between">
                ...
                <button onClick={() => toggleSource(src.id)} className={`w-11 h-6 rounded-full transition-colors relative ${settings.sources_enabled.includes(src.id) ? "bg-lime-700" : "bg-stone-200"}`}>
