...
export default function OpportunityFiltersPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8"><h1 className="text-3xl font-bold">Search Filters</h1><p className="text-stone-500 mt-1">Configure default filters for opportunity searches</p></div>
      ...
      <form onSubmit={saveFilters} className="space-y-6">
        ...
        <div className="p-6 bg-white border border-stone-200 rounded-xl">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-xs text-stone-500 mb-1">Minimum ($)</label><input type="number" value={filters.min_value} onChange={e => setFilters((f: any) => ({ ...f, min_value: e.target.value }))} placeholder="0" className="w-full px-4 py-3 bg-stone-100 border border-stone-300 rounded-lg focus:ring-2 focus:ring-lime-500 focus:outline-none text-sm" /></div>
            <div><label className="block text-xs text-stone-500 mb-1">Maximum ($)</label><input type="number" value={filters.max_value} onChange={e => setFilters((f: any) => ({ ...f, max_value: e.target.value }))} placeholder="No limit" className="w-full px-4 py-3 bg-stone-100 border border-stone-300 rounded-lg focus:ring-2 focus:ring-lime-500 focus:outline-none text-sm" /></div>
          </div>
        </div>
        <div className="p-6 bg-white border border-stone-200 rounded-xl">
          <h2 className="font-semibold mb-4">Set-Aside Types</h2>
          <div className="flex flex-wrap gap-2">{SET_ASIDE_OPTIONS.map(sa => (
            <button key={sa} type="button" onClick={() => toggleSetAside(sa)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${filters.set_asides.includes(sa) ? "bg-lime-700 text-white" : "bg-stone-100 text-stone-600 hover:bg-stone-200"}`}>{sa}</button>
          ))}</div>
        </div>
        <div className="p-6 bg-white border border-stone-200 rounded-xl">
          <h2 className="font-semibold mb-4">Notice Types</h2>
          <div className="flex flex-wrap gap-2">{NOTICE_TYPES.map(nt => (
            <button key={nt} type="button" onClick={() => toggleNoticeType(nt)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${filters.notice_types.includes(nt) ? "bg-blue-600 text-white" : "bg-stone-100 text-stone-600 hover:bg-stone-200"}`}>{nt}</button>
          ))}</div>
        </div>
        <button type="submit" disabled={saving} className="w-full px-6 py-3 bg-lime-700 text-white rounded-lg hover:bg-lime-800 disabled:opacity-50 font-medium">{saving ? "Saving..." : "Save Filters"}</button>
      </form>
    </div>
  );
}
