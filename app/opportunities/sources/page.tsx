...
export default function OpportunitySourcesPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8"><h1 className="text-3xl font-bold">Opportunity Sources</h1><p className="text-stone-500 mt-1">Federal procurement data sources integrated with Harpoon AI</p></div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        <div className="p-4 bg-white border border-stone-200 rounded-xl text-center"><p className="text-2xl font-bold text-lime-700">{SOURCES.filter(s => s.status === "active").length}</p><p className="text-xs text-stone-500 mt-1">Active Sources</p></div>
        <div className="p-4 bg-white border border-stone-200 rounded-xl text-center"><p className="text-2xl font-bold text-blue-600">{SOURCES.length}</p><p className="text-xs text-stone-500 mt-1">Total Sources</p></div>
        <div className="p-4 bg-white border border-stone-200 rounded-xl text-center"><p className="text-2xl font-bold text-purple-600">500K+</p><p className="text-xs text-stone-500 mt-1">Opportunities</p></div>
        <div className="p-4 bg-white border border-stone-200 rounded-xl text-center"><p className="text-2xl font-bold text-amber-400">Real-time</p><p className="text-xs text-stone-500 mt-1">Data Refresh</p></div>
      </div>
      {types.map(type => (
        <div key={type} className="mb-6">
          <h2 className="text-lg font-semibold mb-3 text-lime-700">{type}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {SOURCES.filter(s => s.type === type).map(s => (
              <div key={s.name} className="p-4 bg-white border border-stone-200 rounded-xl">
                <div className="flex items-start justify-between">
                  <div><h3 className="font-semibold text-sm">{s.name}</h3><p className="text-xs text-stone-500 mt-1">{s.desc}</p></div>
                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${s.status === "active" ? "bg-lime-50 text-lime-700" : "bg-stone-100 text-stone-500"}`}>{s.status}</span>
                </div>
                <div className="flex items-center justify-between mt-3"><span className="text-xs text-stone-8000">{s.count}</span>
                  <button onClick={() => router.push(`/marketplaces`)} className="text-xs text-blue-600 hover:underline">View &rarr;</button>
                </div>
              </div>
            ))}
          </div>
