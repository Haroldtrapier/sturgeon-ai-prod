...
export default function SubcontractingPlanPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8"><h1 className="text-3xl font-bold">Subcontracting Plan</h1><p className="text-stone-500 mt-1">Generate FAR-compliant small business subcontracting plans</p></div>
      <div className="grid grid-cols-1 md:grid-cols-5 gap-3 mb-6">
        {SB_CATEGORIES.map(c => (
          <div key={c.category} className="p-3 bg-white border border-stone-200 rounded-xl text-center">
            <p className="text-xl font-bold text-lime-700">{c.goal}</p>
      ...
      <div className="mb-8 p-6 bg-white border border-stone-200 rounded-xl">
        <div className="flex gap-3">
          <input type="number" placeholder="Estimated Contract Value ($)" value={contractValue} onChange={e => setContractValue(e.target.value)} className="flex-1 px-4 py-3 bg-stone-100 border border-stone-300 rounded-lg focus:ring-2 focus:ring-lime-500 focus:outline-none" />
          <button onClick={generatePlan} disabled={generating} className="px-6 py-3 bg-lime-700 text-white rounded-lg hover:bg-lime-800 disabled:opacity-50 font-medium">{generating ? "Generating..." : "Generate Plan"}</button>
        </div>
      </div>
      {plan && (
        <div className="p-6 bg-white border border-lime-200 rounded-xl">
          <h2 className="text-lg font-semibold mb-4 text-lime-700">Generated Subcontracting Plan</h2>
          <pre className="text-sm text-stone-600 whitespace-pre-wrap">{plan}</pre>
        </div>
      )}
    </div>
  );
