...
export default function ProposalEditorPage() {
  ...
  if (!proposal) return (
    <div className="max-w-3xl mx-auto px-4 py-8 text-center">
      <p className="text-stone-500 mb-3">No proposal selected</p>
      <button onClick={() => router.push("/proposals")} className="px-4 py-2 bg-lime-700 text-white rounded-lg text-sm">Go to Proposals</button>
    </div>
  );
    ...
    <div className="flex h-[calc(100vh-4rem)]">
      <div className="w-64 bg-white border-r border-stone-200 overflow-y-auto flex-shrink-0">
        <div className="p-4 border-b border-stone-200">
          ...
          <p className="text-xs text-stone-500 mt-1">{sections.length} sections</p>
        </div>
        <div className="p-2">{sections.map((s: any, i: number) => (
          <button key={i} onClick={() => setActiveSection(i)} className={`w-full text-left px-3 py-2 rounded-lg text-sm mb-1 transition-colors ${activeSection === i ? "bg-lime-700 text-white" : "text-stone-500 hover:bg-stone-100"}`}>{s.title || s.type || `Section ${i + 1}`}</button>
        ))}</div>
      </div>
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-3 border-b border-stone-200">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={saveSection} disabled={saving} className="px-3 py-1.5 bg-lime-700 text-white rounded text-xs hover:bg-lime-800 disabled:opacity-50">{saving ? "Saving..." : "Save"}</button>
          </div>
        </div>
        <div className="flex-1 p-6 overflow-y-auto">
          <textarea value={sections[activeSection]?.content || ""} onChange={e => updateSectionContent(e.target.value)} className="w-full h-full min-h-[400px] px-4 py-3 bg-stone-100 border border-stone-300 rounded-lg focus:ring-2 focus:ring-lime-500 focus:outline-none text-sm resize-none" placeholder="Start writing or use AI Generate to create content..." />
        </div>
      </div>
    </div>
  );
