...
export default function NAICSLookupPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8"><h1 className="text-3xl font-bold">NAICS Code Lookup</h1><p className="text-stone-500 mt-1">Search and analyze NAICS codes for government contracting</p></div>
      <form onSubmit={searchNAICS} className="mb-8 p-6 bg-white border border-stone-200 rounded-xl">
        <div className="flex gap-3">
          <input type="text" value={query} onChange={e => setQuery(e.target.value)} placeholder="Enter NAICS code or describe your services..." className="flex-1 px-4 py-3 bg-stone-100 border border-stone-300 rounded-lg focus:ring-2 focus:ring-lime-500 focus:outline-none" />
      ...
      {result && (
        <div className="mb-8 p-6 bg-white border border-lime-200 rounded-xl">
          <h2 className="text-lg font-semibold mb-3 text-lime-700">NAICS Analysis</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {COMMON_NAICS.map(n => (
          <button key={n.code} onClick={() => setQuery(n.code)} className="p-3 bg-white border border-stone-200 rounded-xl text-left hover:border-lime-600 transition-colors">
            <span className="text-lime-700 font-mono text-sm">{n.code}</span>
            <p className="text-sm mt-0.5">{n.title}</p>
            <p className="text-xs text-stone-8000">Size: {n.size}</p>
          </button>
        ))}
      </div>
    </div>
