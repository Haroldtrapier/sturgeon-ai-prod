...
export default function ResearchAnalyticsPage() {
  ...
  const METRICS = [
    { label: "Total Searches", value: stats.total_searches, color: "text-blue-600" },
    { label: "Saved Opportunities", value: stats.saved_opportunities, color: "text-lime-700" },
    { label: "Proposals Created", value: stats.proposals_created, color: "text-purple-600" },
    { label: "Win Rate", value: `${stats.win_rate}%`, color: "text-yellow-600" },
    { label: "Avg Match Score", value: `${stats.avg_match_score}%`, color: "text-lime-700" },
  ];
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="text-stone-500 mt-1">Track your research activity and pipeline performance</p>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-8">
        {METRICS.map(m => (
          <div key={m.label} className="p-4 bg-white border border-stone-200 rounded-xl text-center">
              ...
              <div key={naics} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <div className="w-24 bg-stone-100 rounded-full h-2">
                    <div className="bg-lime-600 h-2 rounded-full" style={{ width: `${100 - i * 15}%` }} />
                  </div>
                  <span className="text-xs text-stone-500">{100 - i * 15}%</span>
                </div>
              </div>
            ))}
          </div>
            ...
            {stats.top_agencies.map((agency, i) => (
              <div key={agency} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-stone-500">{90 - i * 12}%</span>
                </div>
              </div>
            ))}
          </div>
