  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-8">
        <div><h1 className="text-3xl font-bold">CMMC Guidance</h1><p className="text-stone-500 mt-1">Cybersecurity Maturity Model Certification readiness</p></div>
        <button onClick={runAssessment} disabled={running} className="px-4 py-2 bg-lime-700 text-white rounded-lg hover:bg-lime-800 disabled:opacity-50 font-medium">{running ? "Assessing..." : "Run Assessment"}</button>
      </div>
      <div className="space-y-4 mb-8">
        {CMMC_LEVELS.map(l => (
          <div key={l.level} className="p-6 bg-white border border-stone-200 rounded-xl">
            <div className="flex items-center justify-between mb-3">
              <div><h2 className="text-lg font-semibold"><span className="text-lime-700">{l.level}</span> — {l.title}</h2><p className="text-sm text-stone-500 mt-1">{l.description}</p></div>
              <div className="text-right"><p className="text-2xl font-bold text-lime-700">{l.practices}</p><p className="text-xs text-stone-8000">Practices</p></div>
            </div>
            <div className="flex flex-wrap gap-2 mt-3">{l.domains.map(d => (<span key={d} className="px-2 py-1 bg-stone-100 rounded text-xs text-stone-600">{d}</span>))}</div>
          </div>
        ))}
      </div>
      {assessment && (
        <div className="p-6 bg-white border border-stone-200 rounded-xl">
          <h2 className="text-lg font-semibold mb-4 text-lime-700">CMMC Readiness Assessment</h2>
          <pre className="text-sm text-stone-600 whitespace-pre-wrap">{typeof assessment === "string" ? assessment : JSON.stringify(assessment, null, 2)}</pre>
        </div>
      )}
    </div>
  );
