...
export default function ComplianceCertificationsPage() {
  ...
  const statusStyle = (s: string) => s === "active" ? "bg-lime-50 text-lime-700" : s === "expiring" ? "bg-yellow-50 text-yellow-600" : s === "expired" ? "bg-red-50 text-red-600" : "bg-stone-200 text-stone-8000";
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-stone-500 mt-1">Track certification status for proposal compliance</p>
        </div>
        <button onClick={() => router.push("/certifications")} className="px-4 py-2 bg-lime-700 text-white rounded-lg hover:bg-lime-800 text-sm font-medium">Manage Certifications</button>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
        <div className="p-4 bg-white border border-stone-200 rounded-xl text-center">
          <p className="text-2xl font-bold text-lime-700">{REQUIRED_CERTS.filter(c => getCertStatus(c.type).status === "active").length}</p>
          <p className="text-xs text-stone-500">Active</p>
        </div>
        <div className="p-4 bg-white border border-stone-200 rounded-xl text-center">
          <p className="text-2xl font-bold text-yellow-600">{REQUIRED_CERTS.filter(c => getCertStatus(c.type).status === "expiring").length}</p>
          <p className="text-xs text-stone-500">Expiring Soon</p>
        </div>
        <div className="p-4 bg-white border border-stone-200 rounded-xl text-center">
          <p className="text-2xl font-bold text-red-600">{REQUIRED_CERTS.filter(c => getCertStatus(c.type).status === "expired").length}</p>
          <p className="text-xs text-stone-500">Expired</p>
        </div>
        <div className="p-4 bg-white border border-stone-200 rounded-xl text-center">
          <p className="text-2xl font-bold text-stone-500">{REQUIRED_CERTS.filter(c => getCertStatus(c.type).status === "missing").length}</p>
          <p className="text-xs text-stone-500">Not Obtained</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {REQUIRED_CERTS.map(c => {
          ...
          return (
            <div key={c.type} className="p-4 bg-white border border-stone-200 rounded-xl">
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2">
                  <h3 className="font-semibold text-sm">{c.type}</h3>
                  {c.required && <span className="text-[10px] px-1.5 py-0.5 bg-blue-50 text-blue-600 rounded">Required</span>}
                </div>
              ...
              {s.cert && s.cert.expiry_date && (
                <p className="text-xs text-stone-8000 mt-1">Expires: {new Date(s.cert.expiry_date).toLocaleDateString()}</p>
              )}
            </div>
          );
        })}
