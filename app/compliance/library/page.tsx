...
export default function RegulationLibraryPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8"><h1 className="text-3xl font-bold">Regulation Library</h1><p className="text-stone-500 mt-1">Browse federal contracting regulations and standards</p></div>
      <div className="mb-6 flex flex-col md:flex-row gap-4">
        <input type="text" placeholder="Search regulations..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="flex-1 px-4 py-2 bg-stone-100 border border-stone-300 rounded-lg focus:ring-2 focus:ring-lime-500 focus:outline-none text-sm" />
        <div className="flex flex-wrap gap-2">{categories.map(c => (
          <button key={c} onClick={() => setActiveCategory(c)} className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${activeCategory === c ? "bg-lime-700 text-white" : "bg-stone-100 text-stone-600 hover:bg-stone-200"}`}>{c === "all" ? "All" : c}</button>
        ))}</div>
      </div>
      {aiResult && (
        <div className="mb-6 p-6 bg-white border border-lime-200 rounded-xl">
          <div className="flex justify-between items-start mb-3"><h2 className="text-lg font-semibold text-lime-700">AI Explanation</h2><button onClick={() => setAiResult(null)} className="text-stone-8000 hover:text-stone-900">&times;</button></div>
          <pre className="text-sm text-stone-600 whitespace-pre-wrap">{aiResult}</pre>
        </div>
      )}
      <div className="space-y-3">
        {filtered.map((r, i) => (
          <div key={i} className="p-4 bg-white border border-stone-200 rounded-xl hover:border-stone-300 transition-colors">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <span className="px-2 py-0.5 bg-stone-100 rounded text-xs text-lime-700 font-medium">{r.category}</span>
                  <h3 className="font-medium text-sm">{r.title}</h3>
                </div>
                <p className="text-xs text-stone-8000">{r.citations}</p>
                <p className="text-xs text-stone-500 mt-1">{r.desc}</p>
              </div>
              <button onClick={() => askAboutRegulation(r.title)} disabled={asking} className="ml-3 px-3 py-1 bg-lime-700/10 text-lime-700 rounded text-xs hover:bg-lime-700/20 disabled:opacity-50">Explain</button>
            </div>
          </div>
        ))}
      </div>
