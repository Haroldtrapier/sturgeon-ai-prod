...
export default function BillingPage() {
  ...
  const planHighlights: Record<string, { color: string; badge?: string }> = {
    free: { color: "border-stone-300" },
    pro: { color: "border-lime-600", badge: "Most Popular" },
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="text-stone-500 mt-1">Manage your subscription and billing</p>
      </div>
      ...
      <div className="mb-8 p-6 bg-white border border-stone-200 rounded-xl">
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-2xl font-bold text-lime-700 capitalize">{currentPlan}</p>
            <p className="text-sm text-stone-500 mt-1">
              {subscription?.status === "active" ? "Active subscription" : "Free tier - upgrade for more features"}
            </p>
          </div>
          {currentPlan !== "free" && (
            <button onClick={handleCancel} className="px-4 py-2 bg-red-600/10 text-red-600 rounded-lg hover:bg-red-600/20 text-sm font-medium">Cancel Subscription</button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {plans.map((plan) => {
          const highlight = planHighlights[plan.id] || { color: "border-stone-300" };
          const isCurrent = plan.id === currentPlan;
          return (
            <div key={plan.id} className={`p-6 bg-white border-2 rounded-xl relative ${highlight.color} ${isCurrent ? "ring-2 ring-emerald-400" : ""}`}>
              {highlight.badge && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-lime-700 text-white text-xs font-bold rounded-full">{highlight.badge}</span>
              )}
              <h3 className="text-xl font-bold mt-2">{plan.name}</h3>
              <p className="text-3xl font-bold mt-4">
                ${plan.price}<span className="text-sm text-stone-500 font-normal">/mo</span>
              </p>
              <ul className="mt-6 space-y-3">
                {plan.features.map((f, i) => (
                  <li key={i} className="flex items-center gap-2 text-sm text-stone-600">
                    <span className="text-lime-700">&#10003;</span> {f}
                  </li>
                ))}
              </ul>
              <button
                ...
                className={`w-full mt-6 px-4 py-3 rounded-lg font-medium text-sm ${isCurrent ? "bg-stone-200 text-stone-500 cursor-not-allowed" : plan.id === "free" ? "bg-stone-100 text-stone-500 cursor-not-allowed" : "bg-lime-700 text-white hover:bg-lime-800"} disabled:opacity-50`}
              >
