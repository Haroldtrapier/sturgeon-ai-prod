  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="text-stone-500 mt-1">Configure email notification preferences</p>
      </div>

      <div className="space-y-6">
        {categories.map(cat => (
          <div key={cat} className="p-6 bg-white border border-stone-200 rounded-xl">
            <h2 className="text-sm font-semibold text-stone-600 mb-4">{cat}</h2>
            <div className="space-y-4">
              {TOGGLES.filter(t => t.category === cat).map(t => (
                <div key={t.key} className="flex items-center justify-between">
                  ...
                  <button onClick={() => setSettings(p => ({ ...p, [t.key]: !p[t.key as keyof typeof p] }))} className={`w-11 h-6 rounded-full transition-colors relative ${settings[t.key as keyof typeof settings] ? "bg-lime-700" : "bg-stone-200"}`}>
        ...
        <div className="p-6 bg-white border border-stone-200 rounded-xl">
          <h2 className="text-sm font-semibold text-stone-600 mb-4">Timing</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-stone-500 mb-1">Digest Send Time</label>
              <input type="time" value={settings.digest_time} onChange={e => setSettings(p => ({ ...p, digest_time: e.target.value }))} className="w-full px-3 py-2 bg-stone-100 border border-stone-300 rounded-lg text-sm focus:ring-2 focus:ring-lime-500 focus:outline-none" />
            </div>
            <div>
              <label className="block text-sm text-stone-500 mb-1">Deadline Reminder (days before)</label>
              <select value={settings.deadline_days_before} onChange={e => setSettings(p => ({ ...p, deadline_days_before: parseInt(e.target.value) }))} className="w-full px-3 py-2 bg-stone-100 border border-stone-300 rounded-lg text-sm focus:ring-2 focus:ring-lime-500 focus:outline-none">
